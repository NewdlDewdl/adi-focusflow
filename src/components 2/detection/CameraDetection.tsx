...
interface CameraDetectionProps {
  ...
  onDistraction: () => void;
  sessionPhase: "idle" | "running" | "paused";
}
...
export default function CameraDetection({
  ...
  // Detection Pipeline
  const { state: permissionState, error: permissionError, videoRef, requestPermission } = useWebcamPermission();
  const { result, fps, isLoading, loadingMessage, isReady, humanRef } = useHumanDetection(videoRef);
  ...
  return (
    <>
      ...
      {(permissionState === "idle" || permissionState === "checking") && (
        <PermissionOverlay message="Checking camera access..." />
      )}
      ...
      {(permissionState === "denied" || permissionState === "error") && (
        <PermissionError error={permissionError} requestPermission={requestPermission} />
      )}

      {/* Video + Detection (always rendered when permission granted) */}
      {permissionState === "granted" && (
        <>
          ...
          {/* Paused overlay (only when session is paused) */}
          {sessionPhase === "paused" && (
...
function CameraCanvas({
  ...
  useEffect(() => {
    ...
    const drawOverlay = async () => {
      ...
      try {
        const drawResult = {
          face: (result.faces || []).map((face: any) => ({
            ...face,
        ...
        await human.draw.all(canvas, drawResult);
      } catch {
        const face = result.faces[0];
        if (face) {
          const scaleX = displayWidth / (video.videoWidth || 1280);
          const scaleY = displayHeight / (video.videoHeight || 720);
